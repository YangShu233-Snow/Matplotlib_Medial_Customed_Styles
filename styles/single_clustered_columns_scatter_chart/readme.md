...
categories = [
    ...
    ('Group B', ['CON', 'KO']),   # 第二个大组名称及子组名称
    ('Group C', ['CON', 'KO'])    # 第三个大组名称及子组名称
]

# 3. 原始数据信息（需直接提供包含每个样本的数组）
all_raw_data = [
    [np.array([1150, 1200, 1250, ...]), np.array([3200, 3500, 3600, ...])],  # Group A 的 CON 和 KO 数据
    [np.array([1400, 1500, 1600, ...]), np.array([2100, 2200, 2300, ...])],  # Group B 的 CON 和 KO 数据
    [np.array([1000, 1100, 1200, ...]), np.array([950, 1050, 1150, ...])]    # Group C 的 CON 和 KO 数据
]
# 注：程序会自动基于原始数据计算对应的均值 (mean) 和 标准误 (SEM)

# 4. 显著性标记配置
# 格式要求为：每组分别配置，可在一组内同时指定多个星号标记。
# 元组第一个数字代表要标记的子组，第二表示要标记的星数
stars_marks = [
    [(1, 3)],      # 在 Group A 的第 1 个子组 (KO) 上显示 "***"
    [(1, 2)],      # 在 Group B 的第 1 个子组 (KO) 上显示 "**"  
    []             # Group C 不显示星号
]
